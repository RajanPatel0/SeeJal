...
const StationDetail = () => {
  ...
  const availableWater = (
    (station.currentLevel - station.minLevel) *
  ...
  const generateForecast = () => {
    const lastValue =
      station.data[station.data.length - 1]?.level || station.currentLevel;
    const forecast = [];
    for (let i = 1; i <= 15; i++) {
    ...
    <div className="min-h-screen bg-background py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="text-xs mt-1 text-gray-500">
              Specific Yield: {station.specificYield}
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <code className="text-sm text-blue-800">
              Available Water = (Current Level - Min Level) × Specific Yield ×
              Area × 1000
              <br />= ({station.currentLevel}m - {station.minLevel}m) ×{" "}
              {station.specificYield} × {station.area}km² × 1000
              <br />= <strong>{availableWater} Million Liters</strong>
            </code>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          {activeTab === "recharge" && <RechargeChart data={station.data} />}

          {activeTab === "forecast" && (
            <ForecastChart
              historicalData={station.data}
